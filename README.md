...
*Featured result (post-pass): Asymptotic secret-key rate (bits/pulse) and QBER vs channel loss.*
Plots are generated by `./py -m sat_qkd_lab.run sweep` and written to `figures/`.

Supports **pass-time quality telemetry + post-pass security distillation** under delayed-classical CONOPS (qubits downlinked during pass; reconciliation later via RF). During the pass, use `pass-sweep` / `forecast-run` to estimate viability from observables available in the downlink; compute QBER post-pass via `replay` / experiment harness inputs.
